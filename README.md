...
- **Mutations**: default to `useMutationWithInvalidation(options, keys)` (`client/src/hooks/`). Drop to raw `useMutation` only when the wrapper cannot express the flow — `onSettled`-based invalidation, success-ordering (invalidate before navigating), or one shared invalidation across several mutations — and say why in the inline eslint-disable. Optimistic updates are NOT a reason: pass `onMutate`/`onError` through the wrapper (see `Todos.tsx`). Known, deliberate limitation: the wrapper invalidates in `onSuccess` only; optimistic mutations rely on their `onError` rollback for cache consistency rather than a failure-path refetch.
- **Feature boundaries** (lint-enforced): features import each other only via public barrels (`@/features/<name>`); features never import from `@/routes`; shared components never import features; `server/src/shared` (the isomorphic kernel) imports only `temporal-polyfill`/`zod`/`superjson`/siblings; client code imports runtime values only from `@server/shared`. Feature barrels are re-export-only and must stay cycle-free.
